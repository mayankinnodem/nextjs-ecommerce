...
export default function UserOrdersPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/user/order/user-orders?phone=${encodeURIComponent(phone)}`)
      .then(res => res.json())
      .then(data => {
  ...
  <div key={`${item.productId}-${idx}`} className="flex items-center gap-3 border-b py-1">
    <img
    ...
    <div>
      ...
      <p className="text-sm text-gray-600">Price: ₹{Number(item.price).toLocaleString()}</p>
    </div>
  </div>
))}
